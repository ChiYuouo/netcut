    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-900/45 p-4 backdrop-blur-sm">
      <form className="glass-panel w-full max-w-md rounded-2xl p-6 shadow-2xl" onSubmit={submit}>
        <div className="mb-5 flex items-start justify-between gap-4">
          ...
          {onCancel ? (
            <button
              className="glass-button grid size-9 place-items-center rounded-lg text-slate-500 hover:text-slate-800"
              onClick={onCancel}
              type="button"
            >
              <X size={18} />
        ...
        <h1 className="font-heading text-2xl font-extrabold tracking-normal text-slate-950">
          {mode === 'set' ? '设置访问密码' : '输入访问密码'}
        </h1>
        <p className="mt-2 text-sm leading-6 text-slate-500">
          {mode === 'set'
            ? '设置后，同名剪切板会用“名称 + 密码”加密保存。'
            : '这个名称下的内容已设置密码，请输入后查看。'}
        </p>
